...
const FoodAlertScreen = () => {
  ...
  const loadPosts = async () => {
    try {
      const storedPosts = Platform.OS === 'web' ? localStorage.getItem('foodPosts') : await AsyncStorage.getItem('foodPosts');
      if (storedPosts) {
  ...
  const savePosts = async (updatedPosts) => {
    try {
      ...
      if (Platform.OS === 'web') {
        localStorage.setItem('foodPosts', postsString);
      } else {
        await AsyncStorage.setItem('foodPosts', postsString);
      }
    } catch (error) {
      console.error('Failed to save food posts:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Post Food Sale</Text>
...
export default FoodAlertScreen;
