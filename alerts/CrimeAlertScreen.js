...
const CrimeAlertScreen = () => {
  ...
  const loadPosts = async () => {
    try {
      const storedPosts = Platform.OS === 'web' ? localStorage.getItem('crimePosts') : await AsyncStorage.getItem('crimePosts');
      if (storedPosts) {
        setPosts(JSON.parse(storedPosts));
      }
    } catch (error) {
      console.error('Failed to load crime posts:', error);
    }
  };
  ...
  const savePosts = async (updatedPosts) => {
    try {
      ...
      if (Platform.OS === 'web') {
        localStorage.setItem('crimePosts', postsString);
      } else {
        await AsyncStorage.setItem('crimePosts', postsString);
      }
    } catch (error) {
      console.error('Failed to save crime posts:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Post a Crime Alert</Text>
...
export default CrimeAlertScreen;
