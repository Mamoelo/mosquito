...
const ClothesAlertScreen = () => {
  ...
  const loadPosts = async () => {
    try {
      const storedPosts = Platform.OS === 'web' ? localStorage.getItem('clothesPosts') : await AsyncStorage.getItem('clothesPosts');
      if (storedPosts) {
        setPosts(JSON.parse(storedPosts));
      }
    } catch (error) {
      console.error('Failed to load clothes posts:', error);
    }
  };
  ...
  const savePosts = async (updatedPosts) => {
    try {
      ...
      if (Platform.OS === 'web') {
        localStorage.setItem('clothesPosts', postsString);
      } else {
        await AsyncStorage.setItem('clothesPosts', postsString);
      }
    } catch (error) {
      console.error('Failed to save clothes posts:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Post Clothes Sale</Text>
...
export default ClothesAlertScreen;
